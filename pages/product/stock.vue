...
const onChangeFilterOrderCol = () => {
    if (!filterOrder.value) filterOrder.value = 'asc'
    getProducts()
}
                    ...
                    <template #body="slotProps">
                        <div class="min-w-12 w-12 flex gap-2">
                            <KTBButton icon="pi pi-trash" type="outlined" border-color="border-red-700"
                                ...
                                @click.stop="confirmDeleteProduct(slotProps.data)" />
                        </div>
                    </template>
                </Column>
            </DataTable>
                        ...
                        <template v-if="isTableDataLoading">
                            <div v-for="index in recordPerPage" :key="index" class="w-full">
                                <div class="px-0 py-4 sm:p-4 flex flex-col sm:flex-row items-start gap-4 cursor-pointer"
                                    ...
                                    <KTBLoadingSkeleton class="shadow-md block mx-auto sm:!w-[160px] max-sm:!w-[240px]"
                                        height="106.66px" />
                                    <div
                                        class="w-60 mx-auto flex flex-col gap-4 sm:flex-row justify-between items-start flex-1">
                            ...
                            <div v-for="(  item, index  ) in   slotProps.items  " :key="index" class="w-full">
                                <div class="px-0 py-4 sm:p-4 flex flex-col sm:flex-row items-start gap-4 cursor-pointer"
                                    ...
                                    <div
                                        class="w-60 mx-auto flex flex-col gap-4 sm:flex-row justify-between items-start flex-1">
                                        <div class="flex flex-col items-start gap-3">
                                            <div v-show="isDisplay('name')"
                                                class="text-2xl font-bold text-surface-900 dark:text-surface-0">
                                                {{ item.name }}</div>
                                            <Rating v-show="isDisplay('rating')" :modelValue="item.rating" readonly
                                                :cancel="false"></Rating>
                                            <div class="flex items-center gap-3">
                                        ...
                                        <div
                                            class="w-full flex flex-row justify-between items-center sm:flex-col  sm:items-end gap-3 sm:gap-2">
                                            <span v-show="isDisplay('price')" class="text-2xl font-semibold">${{
                                                item.price }}</span>
                                            <div class="min-w-12 w-12">
